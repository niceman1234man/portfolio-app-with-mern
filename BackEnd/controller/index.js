...
export const getAbout =async(req,res)=>{
    ...
    try {
        ...
        res.status(201).json({success:true,message:"About added successfully"})
    } catch (error) {
        res.status(500).json({success:false,message:"server error"});
        console.log(error);
...
export const getSkill=async(req,res)=>{
    ...
    try {
        await newSkill.save();
        res.status(201).json({success:true,message:"new Skill added succesfully"});
    } catch (error) {
        console.log(error);
        res.status(500).json({success:false,message:"server error"});
...
export const getService=async(req,res)=>{
    ...
    try {
        ...
        res.status(201).json({success:true,message:"new service added successfully"});
    } catch (error) {
        console.log(error);
        res.status(500).json({success:false,message:"server error"});
    }
