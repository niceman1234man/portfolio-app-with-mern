    ...
    const myrepos = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/users/niceman1234man/repos"
        );
  ...
  return (
    <div className="w-full bg-white py-16 text-black font-semibold">
      <div className="max-w-[1240px] mx-auto">
            ...
            recentRepo.map((repo) => (
              <div key={repo.id} className="w-full shadow-xl py-8 px-2">
                ...
                <p>
                  Language:{" "}
                  {languages[repo.id]
                    ? Object.keys(languages[repo.id]).join(", ")
                    : "Loading...."}
                </p>
                <p className="p-2">
        ...
        <button className="bg-[#00df9a] w-[200px] rounded-md font-medium my-6 py-2 mx-auto text-black flex justify-center">
          <a href="https://github.com/niceman1234man?tab=repositories">
            See More Projects
          </a>
        </button>
      </div>
